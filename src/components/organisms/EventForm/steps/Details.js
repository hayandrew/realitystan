...
export default class Details extends Component {
  ...
  isValidated() {
    ...
    if (Object.keys(validateNewInput).every((k) => {
      ...
      if (this.props.getStore().email !== userInput.email || this.props.getStore()
        .gender !== userInput.gender) { // only update store of something changed
      ...
      this.setState(Object.assign(userInput, validateNewInput, this._validationErrors(
        validateNewInput)))
    }
  ...
  _validateData(data) {
    return {
      ...
      emailVal: /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/
        .test(data.email), // required: regex w3c uses in html5
    }
  }
  ...
  render() {
    ...
    return ( <
      ...
      select > <
      div className = {
        notValidClasses.genderValGrpCls
      } > {
      ...
      /> <
      div className = {
        notValidClasses.emailValGrpCls
      } > {
