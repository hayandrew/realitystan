...
class Details extends React.Component {
  ...
  renderInfo(person) {
    ...
    return (
      <div className="float-left">
        <div className="info-name"><label>Name: </label>{person.firstName} {person.lastName}</div>
        <div className="info-age"><label>Age: </label>{person.age}</div>
        { (person.dob) ? <div className="info-dob"><label>Birthday: </label>{person.dob}</div> : ''}
        <div className="info-hometown"><label>Hometown: </label>{person.hometown}</div>
        <div className="info-residence"><label>Residence: </label>{person.residence}</div>
        <div className="info-occupation"><label>Occupation: </label>{person.occupation}</div>
      </div>
    )
  }
  ...
  renderName(type, person) {
    if (type === 'voter') {
      return <a className="detail-hover" data-tip data-for={person.name + 'details'}>{person.firstName}</a>
    }
  }
