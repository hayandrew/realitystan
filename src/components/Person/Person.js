...
class Person extends React.Component {
  ...
  renderVotes(nominee) {
    ...
    let voteCount = this.props.houseguests.filter(person => person.voteId === nominee.id).length
  ...
  getStatusIcons(person) {
    return Object.keys(person).map(key => {
        if (key.match(/is_/)) {
          return (<div key={key} className={'houseguest-stats-icon houseguest-stats-icon--' + key}></div>)
        }
      }
    )
  }
    ...
    return (
      <div className={this.getPersonClass(person, type)} key={personName}>
        <div className="houseguest-details">
          ...
          <div className="houseguest-image" style={{backgroundImage:'url(images/cast/' + person.show_short_id + '/' + person.image}}>
            <div className="houseguest-stats">{ this.getStatusIcons(person) }</div>
          </div>
          { type === 'voter' ? this.props.children : ''}
        </div>
